...
- Event images are populated opportunistically during ingestion: `api/src/events/image-enrichment.ts` fetches each new event's `source_url`, extracts its `og:image` (falling back to `twitter:image`), downloads and uploads it. This is best-effort — a page with no image, an unreachable source, or a download failure just leaves `image_url` null rather than blocking ingestion — and runs with bounded concurrency across a batch rather than serially. `api/src/events/backfill-2026-07-31-event-images.ts` is a one-off script that ran this same enrichment against events that predated the feature.
