...
// CLAUDE.md feedback #50). Deliberately kept byte-identical with
// api/src/camps/format.ts, though — feedback #120's "day off camp" reminder
// email needs the exact same camp-row rendering the in-app list uses, the
// same reason events/format.ts is mirrored into api/src/newsletter/format.ts
// (see that file's own header) — enforced by
// scripts/check-format-parity.mjs, same convention. If you change this
// file, make the same change to api/src/camps/format.ts, or the parity
// check will fail the build. The single-day date-labeling rule
// (Today/Tomorrow/"This <Weekday>", bounded to the current Sunday-Saturday
// calendar week) is genuinely shared with events/format.ts too — both need
