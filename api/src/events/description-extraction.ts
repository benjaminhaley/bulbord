...
- address: a specific, correct location the way Google Maps would resolve it, only if genuinely confirmed — never a guess or approximation. This doesn't have to be a numbered street address: a parade, walk, or street event routinely has no single building address, and a bounded street segment or intersection ("Lincoln Ave from Wellington to Diversey", "Pine Grove Ave & Wellington Ave") is a real, specific, mappable address for this purpose — find and use one of these rather than leaving address empty just because there's no house number. Only fall back to leaving it empty if nothing that specific is findable.
