...
//
// Re-scoped 2026-09-06 (Pipeline Review v2): briefly (2026-09-06, feedback
// #138) this call also produced title/description/location quality checks
// for the admin review page. Moved those to candidate-checks.ts instead,
// which runs only on candidates that survive dedup inside ingestEvents() —
// running the heavier per-field checklist here, on the whole raw batch
// before dedup, would waste calls on candidates thrown away a moment later.
// This function is back to its original single responsibility: keep/reject
// plus a reason either way (the reason is still used for both outcomes —
// see CandidateEvent.relevanceReason for a kept one).
...
Respond with ONLY a JSON array, same length and order as the input, no markdown fences, no explanation. Each element: {"keep": boolean, "reason": string}. "reason" is always required — when keep is true, a short phrase on why the event is relevant; when keep is false, a short phrase naming which of the three rejection triggers applied.`
...
export interface CandidateValidationResult {
  // Each kept candidate carries its own .relevanceReason — see CandidateEvent
  // (ingest.ts). Per-field quality checks now come from candidate-checks.ts.
  kept: CandidateEvent[]
...
// through unfiltered, with no reason set) rather than blocking ingestion —
// this is a second, additional layer on top of the extraction prompt's own
// first-pass filtering, not the only thing standing between a bad listing
// and going live, so a transient failure here shouldn't discard otherwise-
// good events. A genuine gap in judgment (this call actually running but
// making the wrong call) is the real risk this exists to reduce, not
// availability.
export async function filterFamilyRelevantCandidates(candidates: CandidateEvent[]): Promise<CandidateValidationResult> {
  ...
  try {
    ...
    candidates.forEach((candidate, i) => {
      ...
      kept.push({ ...candidate, relevanceReason: reason })
    })
