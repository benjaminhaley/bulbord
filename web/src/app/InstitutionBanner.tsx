...
// own picture on the right as the entry point to their notifications. Once
// per page's IonHeader (Ionic supports stacking multiple IonToolbars in one
// header — the same mechanism EventsPage's segment row already uses),
// rather than hoisted above IonTabs globally, since ion-tabs'
...
export function InstitutionBanner() {
  ...
  // longer carries its own dot either.
    ...
    <IonToolbar style={{ '--background': 'var(--banner-bg)', '--color': 'var(--banner-ink)' } as React.CSSProperties}>
      ...
      >
        ...
        {showNotificationBadge && (
          <BadgeDot corner="bottom-right" label="New notifications" count={unseenNotificationCount} />
        )}
      </div>
    </IonToolbar>
  )
